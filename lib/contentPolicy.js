...
let Policy = exports.Policy =
{
  ...
  init: function()
  {
    ...
    // stylesheet.
    // kontaxis: XXX hack to NOT load and register the stylesheet
    /*
...
let PolicyImplementation =
{
  ...
  shouldLoad: function(contentType, contentLocation, requestOrigin, node, mimeTypeGuess, extra)
  {
    ...
    //let result = true;
    {
      let principal = gSecMan.getNoAppCodebasePrincipal(location);
      gDbService.lookup(principal, "mozpub-track-digest256", 
        function handleEvent(tables)
        {
          /* found in at least one table. do kill it */
          if (tables.length)
          {
            dump("[ABP+kontaxis] TRACK_URL_FOUND '" + 
              location.scheme + "://" + location.hostPort 
              + location.path + "'\n");
            //result = false;
          }
          else
          {
            dump("[ABP+kontaxis] TRACK_URL_NONE '" + 
              location.scheme + "://" + location.hostPort 
              + location.path + "'\n");
          }
        }
    ...
    if (!result)
      dump("[ABP+kontaxis] killing " + 
        location.scheme + "://" + location.hostPort + location.path + "\n");
    ...
    // kontaxis: if we are going to REJECT this request, 
    // do register an on-load handler to hide potential 
    // leftover elements
    //
    // asynchronous to impact shouldLoad() as little as possible
    // This is so much better than having the huge stylesheet 
    // hide the blocked element. 

    // 'abphideprocessed' attribute is checked against the document 
    // object to make sure we only register the on-load handler once. 
    // actually it seems multiple shouldLoad() instances may be running 
    // so we end up with a couple extra handler registrations.

    if (!result && !wnd.document.abphideprocessed)
    {
      wnd.document.abphideprocessed = 1;

      /*
      let _loadHandlerResetter = function(e)
      {
        e.target.abphideprocessed = 0;
      }

      wnd.addEventListener("unload", _loadHandlerResetter);
      */

      let _loadHandlerHidder = function(e)
      {
        dump("[ABP+kontaxis] _loadHandlerHidder is here for " 
          + e.target.location.href + " " + Math.random() + "\n");

        // get domain
        let docDomain = getHostname(e.target.location.href);

        // stupid mode. gives us back some stats though
        if (typeof Prefs.hideElemStats != undefined 
            && Prefs.hideElemStats == true) 
        {
          // get CSS hide selectors from blacklist using domain as index
          let domainSelectors = ElemHide.getSelectorsForDomain(docDomain, true);

          let wildSelectors = ElemHide.getSelectorsForDomain("", true);

          // count how many selectors matched elements in the page
          let validSelectorCount = 0;

          // count how many selectors generated syntax errors
          let erroneousSelectorCount = 0;

          // for each CSS hide selector do find any element matches 
          // and hide those elements
          for (let selector in domainSelectors)
          {
            let elementNodeList = [];

            try 
            {
              // get list of elements matching this selector
              elementNodeList 
                = e.target.querySelectorAll(
                    domainSelectors[selector]);
            }
            // we'll get back a SYNTAX_ERR exception 
            // if the CSS selector has bad syntax
            catch (e)
            {
              if (e.code == e.SYNTAX_ERR)
              {
                erroneousSelectorCount++;
              }
              else
                throw e;
            }

            // if this selector matched any elements, 
            // do count it in the useful ones
            if (elementNodeList.length) 
            {
              dump("[ABP+kontaxis] CSSMATCHES " + docDomain + "\t" 
                + domainSelectors[selector] + "\t" 
                + location.scheme + "://" + location.hostPort 
                + location.path + " " + elementNodeList.length + "\n");

              validSelectorCount++;
            }

            // hide those elements returned by the selector
            for (var i = 0; i < elementNodeList.length; i++)
            {
              // hide the element and its children (right... ? )
              elementNodeList[i].style.display = "none";
              // remove the element and its children
              /*elementNodeList[i].parentNode.removeChild(
                elementNodeList[i]);*/
            }
          }

          dump("[ABP+kontaxis] " + docDomain + " yielded " 
            + validSelectorCount + "," + erroneousSelectorCount 
            + "/" + domainSelectors.length 
            + " useful CSS hide selectors\n");

          // reset counters
          validSelectorCount = 0;
          erroneousSelectorCount = 0;

          // for each CSS hide selector do find any element matches 
          // and hide those elements
          for (let selector in wildSelectors)
          {
            let elementNodeList = [];
          ...
          dump("[ABP+kontaxis] " + docDomain + " yielded* " 
            + validSelectorCount + "," + erroneousSelectorCount 
            + "/" + wildSelectors.length 
            + " useful CSS hide selectors\n");
        }
        // smart mode. no stats :(
        else
        {
          // longest suffix domain match
          let xDomain = docDomain;
          while (1)
          {
            dump("[ABP+kontaxis] _loadHandlerHidder fetching selectors for " 
            + xDomain + " ");

            domainSelectors = ElemHide.getSelectorsForDomain2(xDomain);

            if (domainSelectors) 
              dump(domainSelectors.length + "\n");
            else
              dump("??\n");

            // break on first match
            if (domainSelectors && domainSelectors.length != 0)  break;

            // go one level up. TODO stop before TLD
            let i = xDomain.indexOf(".");

            // we are done
            if (i == -1) break;

            // truncate domain and keep trying
            xDomain = xDomain.substring(i+1);
          }
        
          if (typeof domainSelectors == 'undefined')
            domainSelectors = [];

          // add any global selectors to the domain-specific selectors
          domainSelectors 
            = domainSelectors.concat(ElemHide.getSelectorsForDomain2(""));

          dump("[ABP+kontaxis] " + xDomain + " has now " 
            + domainSelectors.length + " selectors (incl. global)\n");

          // create a stylesheet using those selectors and 
          // set the "display:none" property to hide any matching elements
          let styleEl = e.target.createElement('style');

          styleEl.appendChild(e.target.createTextNode(''));

          e.target.head.appendChild(styleEl);

          let styleSheet = styleEl.sheet;

          styleSheet.insertRule(domainSelectors + " {display: none}", 0);
        }
      }

      //wnd.removeEventListener("load", _loadHandlerHidder);
      wnd.addEventListener("load", _loadHandlerHidder);
    }
