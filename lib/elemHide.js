...
/**
 * Lookup table, selectors by their associated doain
 * @type Object
 */
let selectorByDomain = {__proto__: null};
...
let ElemHide = exports.ElemHide =
{
  ...
  clear: function()
  {
    ...
    selectorByDomain = {__proto__: null}
  ...
  add: function(filter)
  {
    ...
    else
    {
      ...
      for (let d in filter.domains)
      {
        // consider domains that SHOULD be considered for this filter. 
        // domains also contains domains that should NOT be considered. 
        // we do not want those in the filterByDomain array (for now)
        if (filter.domains[d])
        {
          if (!selectorByDomain[d]) selectorByDomain[d] = filter.selector;
          selectorByDomain[d] += "," + filter.selector;
          //dump("[*] '" + d + "' " + filter + "\n");
        }
      }
      // there are no domains for this filter. it applies globally.
      if (!filter.domains)
      {
        if (!selectorByDomain[""]) selectorByDomain[""] = filter.selector;
        selectorByDomain[""] += "," + filter.selector;
        //dump("[*] GLOBAL " + filter + "\n");
      }
      //
  ...
  apply: function()
  {
    return; // kontaxis: XXX hack to NOT load and register the stylesheet
    if (this._applying)
    {
  ...
  getSelectorsForDomain2: function(domain)
  {
    return selectorByDomain[domain.toUpperCase()];
  },
  ...
  getSelectorsForDomain: function(/**String*/ domain, /**Boolean*/ specificOnly)
  {
    ...
    for (let key in filterByKey)
    {
      ...
      // skip global filters in specificOnly mode ONLY 
      // if domain is NOT null or empty. reason is we might 
      // want to do getSelectorsForDomain(null, true) to get 
      // just the global selectors but not the domain-specific ones. 
      // an equivalent call would have been getGlobalSelectors()
      if (specificOnly && (!domains || domains[""]) && domain && domain != "")
